...
export function AcademicSyllabusPage() {
  ...
  const { data: syllabuses, loading, error, execute: fetchSyllabuses } = useApi<SyllabusItem[], []>(
    () => dataService.getSyllabuses(),
    true,
    []
  );
  ...
  const handleDeleteItem = async (item: SyllabusItem) => {
    if (!window.confirm(t('Are you sure you want to delete this syllabus?'))) return;
    try {
      await dataService.deleteSyllabus(item.id);
      showToast(t('Syllabus deleted successfully'), 'success');
      fetchSyllabuses();
    } catch (err: any) {
      showToast(err.message || t('Error deleting syllabus'), 'error');
    }
  };

  const columns: Column<SyllabusItem>[] = [
    { key: 'id', label: '#' },
      ...
      render: (item) => (
        item.file ? (
          <a
            ...
            download={item.file.startsWith('data:') ? `${item.title.replace(/\s+/g, '_')}_syllabus` : undefined}
            onClick={(e) => {
              if (!item.file?.startsWith('data:')) {
                e.preventDefault();
                alert(t('Mock syllabus file attached: ') + item.file);
              }
            }}
        ...
        <div className="flex items-center gap-1.5 flex-nowrap">
          {isAuthorized && (
            <>
              <button
                type="button"
                onClick={() => navigate(`/dashboard/academic/syllabus/edit/${item.id}`)}
                title={t('Edit')}
  ...
  const filterFn = (item: SyllabusItem, searchTerm: string, selectedClass: string) => {
  ...
  if (loading && (!syllabuses || syllabuses.length === 0)) {
    return (
  ...
  if (error) {
    return (
      <div className="p-6">
        <ErrorMessage message={error} onRetry={fetchSyllabuses} />
      </div>
    );
  }
  ...
  return (
    <>
      <GenericTablePage<SyllabusItem>
        titleKey="Syllabus"
        iconName="fa-file-text"
        columns={columns}
        initialData={syllabuses || []}
        filterFn={filterFn}
        showClassFilter={true}
        onAddClick={() => navigate('/dashboard/academic/syllabus/add')}
      />

      {toast && (
        <div
          className={`fixed top-4 right-4 z-[9999] p-4 rounded-lg shadow-md text-white font-bold transition-all duration-300 flex items-center gap-2 animate-fadeIn ${toast.type === 'success' ? 'bg-teal bg-[#1abc9c] shadow-[#1abc9c]/20' : 'bg-iconred bg-red-500 shadow-red-500/20'
            }`}
        >
