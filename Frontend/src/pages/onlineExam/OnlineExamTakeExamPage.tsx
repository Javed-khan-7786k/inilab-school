...
export function OnlineExamTakeExamPage() {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const viewModal = useModal<TakeExamItem>();
  const [items, setItems] = useState<TakeExamItem[]>([]);
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const data = await dataService.getTakeExams();
      setItems(data || []);
    } catch (err: any) {
      setError(err.message || 'Failed to load Take Exam records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string | number) => {
    if (!window.confirm(t('Are you sure you want to delete this Take Exam record?'))) return;
    try {
      await dataService.deleteTakeExam(id);
      fetchItems();
    } catch (err: any) {
  ...
  const filterFn = (item: TakeExamItem, term: string) =>
    item.name.toLowerCase().includes(term.toLowerCase()) ||
    item.examStatus.toLowerCase().includes(term.toLowerCase()) ||
    item.duration.toLowerCase().includes(term.toLowerCase()) ||
  ...
  } = useSearchAndFilter<TakeExamItem>({
    initialData: items,
    filterFn,
  ...
  const handleExport = async (format: string) => {
    const filename = 'Take_Exam_Report';
    switch (format) {
      case 'csv':
  ...
  return (
    <DashboardLayout>
              ...
              <div className="bg-[#f8f9fa] border-b border-[#e7eaec] px-[15px] py-[10px] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 select-none">
                <div className="flex items-center gap-2">
                  <Button
                    variant="success"
                    size="sm"
                    onClick={() => navigate('/dashboard/online-exam/take-exam/add')}
                ...
                <table className="w-full border-collapse">
                  <thead className="bg-[#f8f9fa] border-b border-[#e7eaec] select-none">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider w-32">{t('Duration')}</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider w-40">{t('Date')}</th>
                        ...
                        <tr key={item.id} className="hover:bg-[#f5f5f5] transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center space-x-1.5">
                              ...
                              <button
                                type="button"
                                onClick={() => navigate(`/dashboard/online-exam/take-exam/edit/${item.id}`)}
      ...
      {/* View Take Exam Details Modal */}
      {viewModal.isOpen && viewModal.activeItem && (
        <Modal
          isOpen={viewModal.isOpen}
          onClose={viewModal.close}
          title={viewModal.activeItem.name}
        >
          <div className="space-y-4 text-sm text-gray-700">
            <div className="grid grid-cols-2 gap-4 bg-[#f8f9fa] p-3.5 rounded border border-[#e7eaec]">
              <div>
                <span className="text-xs font-semibold text-muted block uppercase">{t('Exam Name')}</span>
                <span className="font-bold text-gray-800 text-base">{viewModal.activeItem.name}</span>
              </div>
              <div>
                <span className="text-xs font-semibold text-muted block uppercase">{t('Status')}</span>
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-[#ebf5fb] text-[#2980b9] border border-[#a9cce3] mt-1">
                  {viewModal.activeItem.examStatus}
                </span>
              </div>
              <div>
                <span className="text-xs font-semibold text-muted block uppercase">{t('Duration')}</span>
                <span className="font-medium text-gray-700">{viewModal.activeItem.duration}</span>
              </div>
              <div>
                <span className="text-xs font-semibold text-muted block uppercase">{t('Date')}</span>
