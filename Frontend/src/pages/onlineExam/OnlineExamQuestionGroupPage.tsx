...
export function OnlineExamQuestionGroupPage() {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const [items, setItems] = useState<QuestionGroupItem[]>([]);
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const data = await dataService.getQuestionGroups();
      setItems(data || []);
    } catch (err: any) {
      setError(err.message || 'Failed to load Question Groups');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string | number) => {
    if (!window.confirm(t('Are you sure you want to delete this Question Group?'))) return;
    try {
      await dataService.deleteQuestionGroup(id);
      fetchItems();
    } catch (err: any) {
      alert(err.message || 'Failed to delete Question Group');
    }
  };
  ...
  const filterFn = (item: QuestionGroupItem, term: string) =>
  ...
  } = useSearchAndFilter<QuestionGroupItem>({
    initialData: items,
    filterFn,
  ...
  const handleExport = async (format: string) => {
    const filename = 'Question_Group_Report';
    switch (format) {
      case 'csv':
  ...
  return (
    <DashboardLayout>
      <div className="bg-white rounded-[4px] shadow-[0_1px_1px_rgba(0,0,0,.1)] border border-[#e7eaec] overflow-hidden">
        <PageHeaderBar
          titleKey="Question Group"
          iconName="fa-folder"
          breadcrumbLabel="Question Group"
        />
              ...
              <div className="bg-[#f8f9fa] border-b border-[#e7eaec] px-[15px] py-[10px] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 select-none">
                <div className="flex items-center gap-2">
                  <Button
                    variant="success"
                    size="sm"
                    onClick={() => navigate('/dashboard/online-exam/question-group/add')}
                  ...
                  >
                    ...
                    <span>{t('Add a Question Group')}</span>
                  </Button>
                        ...
                        <tr key={item.id} className="hover:bg-[#f5f5f5] transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center space-x-1.5">
                              <button
                                type="button"
                                onClick={() => navigate(`/dashboard/online-exam/question-group/edit/${item.id}`)}
