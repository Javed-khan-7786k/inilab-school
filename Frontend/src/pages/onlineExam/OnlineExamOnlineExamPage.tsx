...
export function OnlineExamOnlineExamPage() {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const [items, setItems] = useState<OnlineExamItem[]>([]);
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const data = await dataService.getOnlineExams();
      setItems(data || []);
    } catch (err: any) {
      setError(err.message || 'Failed to load Online Exams');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTogglePublished = async (id: string | number) => {
    setTogglingId(id);
    try {
      const updated = await dataService.toggleOnlineExamPublished(id);
      setItems((prev) =>
        prev.map((item) => (item.id === id ? { ...item, published: updated.published } : item))
      );
    } catch (err: any) {
      alert(err.message || 'Failed to toggle Published status');
    } finally {
      setTogglingId(null);
    }
  };
  ...
  const handleDelete = async (id: string | number) => {
    if (!window.confirm(t('Are you sure you want to delete this Online Exam?'))) return;
    try {
      await dataService.deleteOnlineExam(id);
      fetchItems();
    } catch (err: any) {
      alert(err.message || 'Failed to delete Online Exam');
    }
  };
  ...
  const filterFn = (item: OnlineExamItem, term: string) =>
    item.examTitle.toLowerCase().includes(term.toLowerCase()) ||
    item.examStatus.toLowerCase().includes(term.toLowerCase()) ||
  ...
  } = useSearchAndFilter<OnlineExamItem>({
    initialData: items,
    filterFn,
    initialSortField: 'examTitle',
  ...
  const exportColumns: ExportColumn[] = [
    { header: 'Exam Title', accessorKey: 'examTitle' },
    { header: 'Exam Status', accessorKey: 'examStatus' },
  ...
  const handleExport = async (format: string) => {
    const filename = 'Online_Exam_Report';
    switch (format) {
      case 'csv':
  ...
  return (
    <DashboardLayout>
      <div className="bg-white rounded-[4px] shadow-[0_1px_1px_rgba(0,0,0,.1)] border border-[#e7eaec] overflow-hidden">
        <PageHeaderBar
          titleKey="Online Exam"
          iconName="fa-folder"
          breadcrumbLabel="Online Exam"
        />
              ...
              <div className="bg-[#f8f9fa] border-b border-[#e7eaec] px-[15px] py-[10px] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 select-none">
                <div className="flex items-center gap-2">
                  <Button
                    variant="success"
                    size="sm"
                    onClick={() => navigate('/dashboard/online-exam/online-exam/add')}
                ...
                <table className="w-full border-collapse">
                  <thead className="bg-[#f8f9fa] border-b border-[#e7eaec] select-none">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider">{t('Exam Title')}</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider w-36">{t('Exam Status')}</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider w-52">{t('Date')}</th>
                      <th className="px-4 py-3 text-center text-xs font-semibold text-[#676a6c] uppercase tracking-wider w-28">{t('Published')}</th>
                    ...
                    {paginatedItems.length === 0 ? (
                      ...
                      paginatedItems.map((item, idx) => (
                        <tr key={item.id} className="hover:bg-[#f5f5f5] transition-colors">
                          ...
                          <td className="px-4 py-3 font-medium text-gray-800">{item.examTitle}</td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold bg-[#ebf5fb] text-[#2980b9] border border-[#a9cce3]">
                              {item.examStatus}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-gray-600 font-normal">{item.date || '-'}</td>
                          <td className="px-4 py-3 text-center">
                            <button
                              type="button"
                              onClick={() => handleTogglePublished(item.id)}
                              disabled={togglingId === item.id}
                              className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#1ab394] focus:ring-offset-2 ${
                                item.published ? 'bg-[#1ab394]' : 'bg-gray-300'
                              } disabled:opacity-50`}
                              title={item.published ? t('Published (Click to Unpublish)') : t('Unpublished (Click to Publish)')}
                            >
                              <span
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center space-x-1.5">
                              <button
                                type="button"
                                onClick={() => navigate(`/dashboard/online-exam/online-exam/edit/${item.id}`)}
