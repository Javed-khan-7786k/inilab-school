...
export const AddQuestionGroupPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditMode && id) {
      const loadItem = async () => {
        try {
          const all = await dataService.getQuestionGroups();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && id) {
        await dataService.updateQuestionGroup(id, { title: title.trim() });
        showToast(t('Question Group updated successfully'), 'success');
      } else {
        await dataService.addQuestionGroup({ title: title.trim() });
        showToast(t('Question Group added successfully'), 'success');
      }

      setTimeout(() => {
        navigate('/dashboard/online-exam/question-group');
      }, 800);
    } catch (err: any) {
      showToast(err.message || t('Failed to save Question Group'), 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <PageHeaderBar
        titleKey={isEditMode ? 'Edit Question Group' : 'Add a Question Group'}
        iconName="fa-folder"
        breadcrumbLabel={isEditMode ? 'Edit Question Group' : 'Add Question Group'}
      />
        ...
        <div className="bg-white rounded-[3px] border border-[#e1e1e1] shadow-sm max-w-2xl mx-auto overflow-hidden">
          <div className="bg-[#fcfcfc] border-b border-[#e1e1e1] px-6 py-4 flex items-center justify-between">
            <h2 className="text-[16px] font-semibold text-[#444] m-0">
              {isEditMode ? t('Edit Question Group') : t('Add a Question Group')}
            </h2>
            <Button
              type="button"
              variant="secondary"
              size="sm"
              onClick={() => navigate('/dashboard/online-exam/question-group')}
            >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              ...
              <div className="sm:w-3/4">
                <Input
                  ...
                  placeholder="e.g. Reasoning, Math, GRE..."
                  className="w-full"
                />
