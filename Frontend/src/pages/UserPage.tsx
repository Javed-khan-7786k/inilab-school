...
export const UserPage: React.FC = () => {
  ...
  const { data: users, loading, error, execute: fetchUsers } = useApi<UserItem[], []>(
  ...
  const handleDeleteItem = async (item: UserItem) => {
    if (!window.confirm(t('Are you sure you want to delete this user record?'))) return;
    try {
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center gap-1.5 flex-nowrap">
          <button
            type="button"
            onClick={() => navigate(`/dashboard/view/user/${item.id}`)}
  ...
  const filterFn = (item: UserItem, searchTerm: string) => {
    return (
      searchTerm === '' ||
