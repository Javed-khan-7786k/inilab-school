...
export const NoticePage: React.FC = () => {
  const { t } = useLanguage();
  const noticeModal = useModal<NoticeItem>();
  const addEditModal = useModal<NoticeItem>();
  ...
  const [formData, setFormData] = useState<Omit<NoticeItem, 'id'>>({
    title: '',
  ...
  const fetchNotices = async () => {
    ...
    try {
      ...
      const filtered = isAdmin
        ? data
        : data.filter(n => !n.targetRoles || n.targetRoles.length === 0 || n.targetRoles.includes(userRole));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (addEditModal.activeItem) {
        await dataService.updateNotice(addEditModal.activeItem.id, formData);
      } else {
  ...
  const toggleRole = (role: string) => {
    setFormData((prev: Omit<NoticeItem, 'id'>) => ({
      ...prev,
  ...
  const filterFn = (item: NoticeItem, term: string) =>
    item.title.toLowerCase().includes(term.toLowerCase()) ||
    item.notice.toLowerCase().includes(term.toLowerCase());

  const {
    searchTerm,
    setSearchTerm,
    sortedData: filteredNotices
  } = useSearchAndFilter<NoticeItem>({
    initialData: notices,
    filterFn,
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filteredNotices.length, searchTerm]);
  ...
  const exportColumns: ExportColumn[] = [
    ...
    { header: 'Notice', accessorKey: 'notice' },
  ];

  const handleCopy = () => {
    handleCopyToClipboard(filteredNotices, exportColumns);
  };

  const handleExport = async (format: string) => {
    const filename = "Notice_Report";
    switch (format) {
      case 'csv':
        handleExportCsv(filteredNotices, exportColumns, filename);
        break;
      case 'excel':
        try {
          console.log(filteredNotices, exportColumns, filename)
          await exportExcelWithImages(filteredNotices, exportColumns, filename, {
            templateUrl: '/template.xlsx',
          });
        } catch (error) {

          await exportExcelFromTemplate(filteredNotices, exportColumns, filename, {
            templateUrl: '/template.xlsx',
            startRow: 2,
          });
        }
      ...
      case 'pdf':
        try {
          ...
          await exportPdf(filteredNotices, exportColumns, filename);
    ...
    <DashboardLayout>
      <div className="bg-white rounded-[4px] shadow-[0_1px_1px_rgba(0,0,0,.1)] border border-[#e7eaec] overflow-hidden">
        <PageHeaderBar
          ...
          //   <Button variant="success" size="sm" onClick={openAddModal} className="flex items-center gap-2">
          //     <Icon name="fa-plus" /> {t("Add Notice")}
          //   </Button>
          // )}
        />
          ...
          {loading && notices.length === 0 ? (
            ...
            <div className="bg-[#f8f9fa] border-b border-[#e7eaec] px-[15px] py-[10px] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 select-none">
              ...
              <div className="flex items-center gap-2">
                ...
                {isAdmin && (
            <Button variant="success" size="sm" onClick={openAddModal} className="border border-[#dfe6e9] rounded px-3 py-1.5 text-[13px] text-dark focus:outline-none focus:ring-1 focus:ring-accent hover:bg-white bg-white shadow-sm">
              <Icon name="fa-plus" className="text-[13px] text-muted" /><span className="text-[13px] text-muted">{t("Add Notice")}</span> 
            </Button>
          )}
              ...
              <table className="w-full border-collapse">
                <thead className="bg-[#f8f9fa] border-b border-[#e7eaec] select-none">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wider">{t("Notice")}</th>
                  ...
                  {filteredNotices.length === 0 ? (
                    ...
                    paginatedNotices.map((item, index) => (
                      <tr key={item.id} className="hover:bg-[#fafafa] transition-colors text-dark">
                        ...
                        <td className="px-4 py-3 text-sm text-muted max-w-md truncate" title={item.notice}>{item.notice}</td>
                        <td className="px-4 py-3 text-sm">
                          <div className="flex gap-1.5">
                            <Button
                              variant="success"
                              size="sm"
                              onClick={() => noticeModal.open(item)}
            ...
            <div className="px-4 py-3 border-t border-[#e7eaec] select-none bg-[#fdfdfd]">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-[13px] text-muted">
                  {t("Showing")} {filteredNotices.length > 0 ? startIndex + 1 : 0} {t("to")}{' '}
                  {Math.min(startIndex + itemsPerPage, filteredNotices.length)} {t("of")} {filteredNotices.length} {t("entries")}
                </div>
                <div className="flex gap-2">
                  <Button
                    variant="secondary"
      ...
      <Modal
        ...
        title={noticeModal.activeItem?.title || "Notice Details"}
        maxWidthClass="max-w-lg"
      >
        {noticeModal.activeItem && (
          <div className="p-6 space-y-4">
            <div className="flex justify-between items-center text-[12px] text-muted border-b border-[#f0f0f0] pb-2">
              <span>{t("Notice Date")}: {noticeModal.activeItem.date}</span>
              <div className="flex gap-1">
                {(noticeModal.activeItem.targetRoles || []).map((role: string) => (
            ...
            <p className="text-[14px] text-dark leading-relaxed whitespace-pre-line">
              {noticeModal.activeItem.notice}
            </p>
            <div className="pt-2 flex justify-end">
              <Button variant="success" onClick={noticeModal.close}>
                {t("Close")}
              </Button>
            </div>
          </div>
      ...
      <Modal
        ...
        title={addEditModal.activeItem ? t("Edit Notice") : t("Add Notice")}
      ...
      >
        <form onSubmit={handleSave} className="p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="block text-[13px] font-medium text-dark">{t("Notice Content")} <span className="text-red-500">*</span></label>
            <textarea
              required
              rows={6}
