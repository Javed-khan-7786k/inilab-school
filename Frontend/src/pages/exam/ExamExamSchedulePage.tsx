...
export function ExamExamSchedulePage() {
  ...
  const { data: schedules, loading, error, execute: fetchSchedules } = useApi<ExamScheduleItem[], []>(
    () => dataService.getExamSchedules(),
    true,
    []
  );
  ...
  const handleDeleteItem = async (item: ExamScheduleItem) => {
    if (!window.confirm(t('Are you sure you want to delete this exam schedule?'))) return;
    try {
      await dataService.deleteExamSchedule(item.id);
      showToast(t('Exam schedule deleted successfully'), 'success');
      fetchSchedules();
    } catch (err: any) {
      showToast(err.message || t('Error deleting exam schedule'), 'error');
    }
  };

  const columns: Column<ExamScheduleItem>[] = [
    { key: 'id', label: '#' },
        ...
        <div className="flex items-center gap-1.5 flex-nowrap">
          {isAuthorized && (
            <>
              <button
                type="button"
                onClick={() => navigate(`/dashboard/exam/schedule/edit/${item.id}`)}
                title={t('Edit')}
  ...
  const filterFn = (item: ExamScheduleItem, searchTerm: string, selectedClass: string) => {
    const matchesSearch =
      searchTerm === '' ||
  ...
  return (
    <>
      <GenericTablePage<ExamScheduleItem>
        titleKey="Exam Schedule"
        iconName="fa-calendar"
        columns={columns}
        initialData={schedules || []}
        filterFn={filterFn}
        showClassFilter={true}
        onAddClick={() => navigate('/dashboard/exam/schedule/add')}
      />
