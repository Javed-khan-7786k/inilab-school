...
import { getAvailableStreamsForClass } from "../../Utils/streamService";
...
export const EnquiryFormPage: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const isSenior = ["Class 9", "Class 10", "Class 11", "Class 12", "9", "10", "11", "12"].includes(applyingClass);
    if (isSenior && !stream) tempErrors.stream = t("Stream selection is required for senior classes");
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    setApplyingClass(val);
                    const isSenior = ["Class 9", "Class 10", "Class 11", "Class 12", "9", "10", "11", "12"].includes(val);
                    if (!isSenior) {
                      setStream("");
                    }
                  }}
                ...
                {/* DYNAMICALLY GENERATED STREAM FIELD FOR CLASS 9, 10, 11, 12 */}
                {["Class 9", "Class 10", "Class 11", "Class 12", "9", "10", "11", "12"].includes(applyingClass) && (
                  <div className="animate-field-expand">
                    <Select
                      label={t("Select Stream Specialization")}
                      value={stream}
                      onChange={(e) => setStream(e.target.value)}
                      error={errors.stream}
                      requiredField
                      options={[
                        { value: "", label: "Choose Stream Option" },
                        ...getAvailableStreamsForClass(applyingClass).map((st) => ({
                          value: st.name,
