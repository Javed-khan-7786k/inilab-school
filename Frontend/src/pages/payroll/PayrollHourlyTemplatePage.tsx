...
export function PayrollHourlyTemplatePage() {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const [items, setItems] = useState<HourlyTemplateItem[]>([]);
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const data = await dataService.getHourlyTemplates();
      setItems(data || []);
    } catch (err: any) {
      setError(err.message || 'Failed to load Hourly Templates');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string | number) => {
    if (!window.confirm(t('Are you sure you want to delete this Hourly Template?'))) return;
    try {
      await dataService.deleteHourlyTemplate(id);
      fetchItems();
    } catch (err: any) {
      alert(err.message || 'Failed to delete Hourly Template');
    }
  };
  ...
  const filterFn = (item: HourlyTemplateItem, term: string) =>
    item.hourlyGrade.toLowerCase().includes(term.toLowerCase()) ||
    item.hourlyRate.toLowerCase().includes(term.toLowerCase());

  const {
    searchTerm,
    setSearchTerm,
    sortedData: filteredItems,
  } = useSearchAndFilter<HourlyTemplateItem>({
    initialData: items,
    filterFn,
    initialSortField: 'hourlyGrade',
  ...
  const exportColumns: ExportColumn[] = [
    { header: 'Hourly Grades', accessorKey: 'hourlyGrade' },
  ...
  const handleExport = async (format: string) => {
    const filename = 'Hourly_Template_Report';
    switch (format) {
      case 'csv':
  ...
  return (
    <DashboardLayout>
      <div className="bg-white rounded-[4px] shadow-[0_1px_1px_rgba(0,0,0,.1)] border border-[#e7eaec] overflow-hidden">
        <PageHeaderBar
          titleKey="Hourly Template"
          iconName="fa-folder"
          breadcrumbLabel="Hourly Template"
        />
              ...
              <div className="bg-[#f8f9fa] border-b border-[#e7eaec] px-[15px] py-[10px] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 select-none">
                <div className="flex items-center gap-2">
                  <Button
                    variant="success"
                    size="sm"
                    onClick={() => navigate('/dashboard/payroll/hourly-template/add')}
                  ...
                  >
                    ...
                    <span>{t('Add a Hourly template')}</span>
                  </Button>
                ...
                <table className="w-full border-collapse">
                  <thead className="bg-[#f8f9fa] border-b border-[#e7eaec] select-none">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider">{t('Hourly Grades')}</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#676a6c] uppercase tracking-wider">{t('Hourly Rate')}</th>
                    ...
                    {paginatedItems.length === 0 ? (
                      ...
                      paginatedItems.map((item, idx) => (
                        <tr key={item.id} className="hover:bg-[#f5f5f5] transition-colors">
                          ...
                          <td className="px-4 py-3 font-bold text-gray-800">{item.hourlyGrade}</td>
                          <td className="px-4 py-3 font-semibold text-[#1ab394]">{item.hourlyRate}</td>
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center space-x-1.5">
                              <button
                                type="button"
                                onClick={() => navigate(`/dashboard/payroll/hourly-template/edit/${item.id}`)}
